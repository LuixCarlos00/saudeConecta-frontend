...
this.prontuarioDentistaApi.buscarQuestionarioSaude(consultaId)
  ...
  GerarPDF(): void {
    ...
    doc.save(`Questionario_Saude_${this.codigoProntuario || this.consultaId}_${new Date().toISOString().split('T')[0]}.pdf`);
  }
  ...
  private pdfCabecalho(doc: any, pw: number, m: number, y: number): void {
    ...
    doc.text('QUESTIONÁRIO DE SAÚDE', pw / 2, y + 1, { align: 'center' });
    doc.setFontSize(7.5);
