...
@Component({
  selector: 'app-planejamento-medico',
  templateUrl: './planejamento-medico.component.html',
  styleUrls: ['./planejamento-medico.component.css'],
})
export class PlanejamentoMedicoComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<PlanejamentoMedicoComponent>,
...
GerarPDF(): void {
  ...
  doc.text('PLANEJAMENTO TERAPÊUTICO', pageWidth / 2, 16, { align: 'center' });
  doc.setFontSize(8);
  ...
  if (this.planejamentos.length > 0) {
    ...
    doc.text('STATUS', margin + colWidths[0] + colWidths[1] + colWidths[2] + 2, y + 4.5);
    y += 7;
    ...
    for (const item of this.planejamentos) {
      ...
      doc.text(proc, margin + colWidths[0] + 2, y + 4);
      doc.text(`R$ ${(item.valor || 0).toFixed(2)}`, margin + colWidths[0] + colWidths[1] + 2, y + 4);
      doc.text(item.statusAssinatura || 'PENDENTE', margin + colWidths[0] + colWidths[1] + colWidths[2] + 2, y + 4);
    ...
    doc.text(`R$ ${this.totalPlanejamento.toFixed(2)}`, margin + colWidths[0] + colWidths[1] + 2, y + 5);
    y += 12;
  }
  ...
  doc.save(`Planejamento_Terapeutico_${this.codigoProntuario}_${new Date().toISOString().split('T')[0]}.pdf`);
}
