...
export class PlanejamentoMedicoComponent implements OnInit {
  ...
  private pdfCabecalho(doc: any, pw: number, m: number, y: number): void {
    ...
    doc.text('PLANEJAMENTO TERAPÊUTICO', pw / 2, y + 1, { align: 'center' });
    doc.setFontSize(7.5);
  ...
  private pdfRodape(doc: any, pw: number, ph: number): void {
    ...
    for (let i = 1; i <= total; i++) {
      ...
      doc.text(
        `Página ${i} de ${total} | Emitido em ${this.getDataAtual()} às ${this.getHoraAtual()} | Documento clínico - Confidencial`,
